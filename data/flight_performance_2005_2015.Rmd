...
ggplot(data = top_five_delayed, aes(x=carrier_name, ave_delayed))+
   ...
   ggtitle("Average number of delayed flights from Jan 2005 to Jan 2015")
...
average_delay_summary <- delay_summary %>%
  group_by(carrier_name)%>%
  summarize(arrival_delay = sum(arr_delay), 
            carrier_delay = sum(carrier_delay), 
            weather_delay = sum(weather_delay), 
            nas_delay = sum(nas_delay), 
            security_delay = sum(security_delay), 
...
According to the two generated plots, its obvious that we don't have a lot of information regarding SkyWest airlines and Northwest airlines and thus their plots can be removed from the overall plot. Moreover, it appears that the data of the year 2015 is missing a lot so it would be best to exclude this year from the final plots. 
...
+ The top five airlines with the largest average number of delayed flights are: Southwest Airlines, Skywest Airlines, Northwest Airlines, American Eagles Airlines, and American Airlines. 

It is very interesting that American Arlines and American Eagles Airlines both made their names in the two lists. According to the performance evaluation made by subsetting info from the top five delayed airlines, it appears that American Airlines and American Eagles airlines do have a drop in the total number of their arrivals as their on-time proportion decreases. 


```{r}
##Create new CSV
write.tsv(top_five_delayed, file="top_five_delay.csv", row.names = FALSE)
